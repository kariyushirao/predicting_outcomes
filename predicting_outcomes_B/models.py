...
class Constants(BaseConstants):
	name_in_url = 'whatsnext_ubin_13119'
	...
	def creating_session(self):
		...
		if self.round_number == 1:
			for p in self.get_players():
				...
				# append targets to fillers list
...
class Player(BasePlayer):
	...
	comprehension5b = models.IntegerField(
		choices=[[1, 'The machine DOES NOT roll that ball back into the cage before spinning and drawing the next ball'], 
				[2, 'The machine rolls that ball into a DIFFERENT cage before spinning and drawing the next ball'],
				[3, 'The machine DOES roll that ball back into the cage before spinning and drawing the next ball']],
		widget=widgets.RadioSelect
		)
	# analyst questions
	...
	prediction_as = models.IntegerField(
		choices=[[1, 'UP'],[2, 'DOWN']],
		widget=widgets.RadioSelect
		)
	prediction_b = models.IntegerField(
